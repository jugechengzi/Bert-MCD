...
from train_util_bert_bcr import train_bert_classifier
...
def parse():
    ...
    parser.add_argument('--save_path',
                        type=str,
                        default='./trained_model/classifier',
...
annotation_sampler=torch.utils.data.distributed.DistributedSampler(annotation_data)




train_loader = DataLoader(train_data, batch_sampler=train_batch_sampler,pin_memory=True)

dev_loader = DataLoader(dev_data, sampler=dev_sampler,pin_memory=True,batch_size=args.batch_size)

annotation_loader = DataLoader(annotation_data, sampler=annotation_sampler,pin_memory=True,batch_size=args.batch_size)

######################
# load model
######################
writer=SummaryWriter(args.writer)
model=Bert_classfier(args)
model.to(device)

#保存和加载第一个的权重
if args.rank==0:
    torch.save(model.state_dict(),'./init_model/{}_as_{}.pth'.format(args.cell_type,args.aspect))
dist.barrier()

model.load_state_dict(torch.load('./init_model/{}_as_{}.pth'.format(args.cell_type,args.aspect),map_location=device))
model=torch.nn.parallel.DistributedDataParallel(model,device_ids=[int(args.gpu_list[args.rank])])
...
para_pred_encoder=filter(lambda p: p.requires_grad, model.module.pred_encoder.parameters())
para_pred_fc=filter(lambda p: p.requires_grad, model.module.pred_fc.parameters())
para_pred_layernorm=filter(lambda p: p.requires_grad, model.module.layernorm2.parameters())
# para=[
...
for epoch in range(args.epochs):
    ...
    model.train()
    precision, recall, f1_score, accuracy = train_bert_classifier(model,opt_pred, train_loader, device, args,mode='train')
    ...
    precision, recall, f1_score, accuracy=train_bert_classifier(model, opt_pred, dev_loader, device, args,mode='dev')
    if args.rank==0:
        print("Validate")
    ...
    precision, recall, f1_score, accuracy= train_bert_classifier(model, opt_pred, annotation_loader, device, args,mode='test')
    if args.rank==0:
        print("Annotation")
...
if args.rank==0:
    if os.path.exists('./init_model/{}_as_{}.pth'.format(args.cell_type,args.aspect)) is True:
        os.remove('./init_model/{}_as_{}.pth'.format(args.cell_type,args.aspect))
